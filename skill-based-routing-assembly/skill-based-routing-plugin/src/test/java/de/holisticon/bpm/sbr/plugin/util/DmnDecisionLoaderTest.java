...
public class DmnDecisionLoaderTest {
  ...
  public void gets_fullPath() {
    File file = new DmnDecisionLoader.Key("bar" + File.separator + "foo", "").getFile(new File(File.separator));
    // fix for Windows UNC paths
    if (((String) System.getProperties().get("os.name")).contains("Windows")) {
      assertThat(file.getAbsolutePath()).isEqualTo("\\\\bar" + File.separator + "foo.dmn");
    } else {
      assertThat(file.getAbsolutePath()).isEqualTo("bar" + File.separator + "foo.dmn");
    }
  }
